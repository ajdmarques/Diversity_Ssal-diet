...
simper(Y.data, 
       ...
       permutations=1000)->sim1[[j]]
summary(sim1[[j]])

##Identify the names of the comparisons
names(sim1[[j]]) -> sim.names
...
for (i in seq_along(sim.names)){
  ...
  as.data.frame(summary(sim1[[j]])[i]) -> sim.df[[i]]
