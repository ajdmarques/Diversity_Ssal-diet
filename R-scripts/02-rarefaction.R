...
merged.table %>%
  na_if('NA') %>%
  mutate(prey.id = coalesce(Family,Order,Class,Phylum)) -> merged.table
...
## run iNEXT
out1 <- iNEXT(incidence_raw, q = 0, datatype = 'incidence_raw')
write.csv(out1$AsyEst, paste0(output.dir,names(group.list)[j],"_richness.csv"))
 
out2 <- iNEXT(incidence_freq, q = 0, datatype = 'incidence_freq')

##Run iNEXT
ggiNEXT(out1, type = 1) -> inext.plot.unit.div
ggiNEXT(out1, type = 2) -> inext.plot.unit.cov
ggiNEXT(out1, type = 3) -> inext.plot.cov.div
jpeg(paste0(output.dir,names(group.list)[j],".inext.plot.jpeg"))
ggpubr::ggarrange(inext.plot.unit.div,
                  inext.plot.unit.cov,
                  inext.plot.cov.div,
                  common.legend = T,
                  nrow = 3)
...
# Save image
save.image(paste0(main.dir,"02-diet.R"), compress = "gzip")
